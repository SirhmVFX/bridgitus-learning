...
import { getAnnouncementsForStudent, type Announcement } from "@/lib/firestore";
import {
...
export default function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (student?.grade) {
      getAnnouncementsForStudent(student.grade).then(setAnnouncements).catch(() => { });
    }
  }, [student?.grade]);
    ...
    <div className="flex flex-col h-full bg-secondary-color text-white">
      ...
      <div className="px-5 py-5 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] uppercase tracking-widest text-blue-300">Bridgitus</p>
      ...
      <div className="px-5 py-4 border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-primary-color/30 flex items-center justify-center shrink-0">
            {student?.avatar
              ? <Image src={student.avatar} alt="avatar" width={40} height={40} className="object-cover w-10 h-10" />
              : <span className="text-white font-bold text-sm">{student?.firstName?.[0] ?? "S"}</span>}
          </div>
          <div className="overflow-hidden">
            <p className="text-sm font-semibold truncate">{student?.firstName} {student?.lastName}</p>
            <p className="text-xs text-blue-300 truncate">{student?.studentId}</p>
          </div>
        </div>
        <div className="mt-2 flex items-center gap-2 flex-wrap">
          {student?.grade && <span className="bg-primary-color/30 px-3 py-0.5 text-xs text-blue-200">Grade {student.grade}</span>}
          {isPaid
            ? <span className="bg-emerald-500/30 text-emerald-300 text-xs px-2 py-0.5 font-semibold">✓ PAID</span>
            : <span className="bg-amber-500/30 text-amber-300 text-xs px-2 py-0.5 font-semibold">PAYMENT PENDING</span>}
        </div>
      </div>
      ...
      <div className="p-4 border-t border-white/10">
        <button onClick={handleSignOut} className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-white/60 hover:text-white hover:bg-white/5 transition-all">
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-40 bg-white border-b border-gray-200 px-4 lg:px-6 h-14 flex items-center justify-between">
          <button className="lg:hidden p-2 text-gray-600 hover:text-gray-900" onClick={() => setSidebarOpen(true)}>
            <MdMenu size={22} />
          </button>
          ...
          <div className="flex items-center gap-2">
              ...
              {notifOpen && (
                <div className="absolute right-0 top-full mt-1 w-80 sm:w-96 bg-white border border-gray-200 z-50 max-h-96 overflow-y-auto">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    ...
                    <button onClick={() => setNotifOpen(false)} className="text-gray-400 hover:text-gray-600"><MdClose size={16} /></button>
                  </div>
                  {announcements.length === 0 ? (
                    <div className="px-4 py-8 text-center">
                      <MdCampaign size={32} className="mx-auto text-gray-300 mb-2" />
                    ...
                    <div>
                      {announcements.map((a) => (
                        <div key={a.id} className={`border-b border-gray-50 px-4 py-3 last:border-0 ${a.pinned ? "bg-amber-50" : ""}`}>
                          <div className="flex items-start gap-2">
                            {a.pinned
                              ? <MdPushPin size={14} className="text-amber-500 shrink-0 mt-0.5" />
                              : <MdCampaign size={14} className="text-secondary-color shrink-0 mt-0.5" />}
                            <div>
            ...
            <div className="w-8 h-8 bg-secondary-color flex items-center justify-center">
              <span className="text-white text-xs font-bold">{student?.firstName?.[0] ?? "S"}</span>
            </div>
          </div>
        </header>
        ...
        {!isPaid && !FREE_PATHS.some((p) => pathname.startsWith(p)) && (
          <div className="bg-amber-50 border-b border-amber-200 px-4 lg:px-6 py-2.5 flex items-center justify-between gap-4">
            <p className="text-xs text-amber-800 font-medium flex items-center gap-1.5">
              <MdLock size={13} /> Your portal access is limited until payment is complete.
            </p>
            <Link href="/portal/payment" className="text-xs font-bold text-amber-700 hover:underline shrink-0">Pay now →</Link>
          </div>
        )}
