  ...
  return [
    {
      ...
      earned: materialsCompleted >= 1,
    },
    {
      id: "five_materials",
      icon: "📚",
      label: "Bookworm",
      description: "Completed 5 materials",
      earned: materialsCompleted >= 5,
    },
    {
      id: "first_test",
      icon: "🎯",
      ...
      earned: approvedAttempts.length >= 1,
    },
    {
      id: "five_tests",
      icon: "📝",
      ...
      earned: approvedAttempts.length >= 5,
    },
    {
      id: "ten_tests",
      icon: "🏅",
      ...
      earned: approvedAttempts.length >= 10,
    },
    {
      id: "first_pass",
      icon: "✅",
    ...
    },
    {
      ...
      icon: "🏆",
      label: "High Achiever",
      description: "Avg score 80%+ across 3+ tests",
      earned: avgScore >= 80 && approvedAttempts.length >= 3,
    },
    {
      id: "assignment_pro",
      icon: "🎓",
      ...
      earned: assignmentsCompleted >= 10,
    },
  ];
}
...
export default function ProgressPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [att, prog, ass, mats, comps, gapData, practiceData] =
        await Promise.all([
  ...
  const avgScore =
    approvedAttempts.length > 0
      ? Math.round(
          approvedAttempts.reduce((s, a) => s + a.percentage, 0) /
  ...
  const passRate =
    approvedAttempts.length > 0
      ? Math.round(
          (approvedAttempts.filter((a) => a.passed).length /
            approvedAttempts.length) *
            100
        )
      : 0;
  ...
  const doneMats = materials.filter((m) =>
    isMaterialCompleted(completions, m.id!)
  ).length;
  ...
  const credibilityScore = Math.min(
    ...
    Math.round(
      avgScore * 0.35 +
        passRate * 0.25 +
        matPct * 0.2 +
        Math.min(assignmentsCompleted * 4, 20)
    )
  );
  const credLevel =
    credibilityScore >= 80
      ? { label: "Outstanding", color: "#22c55e" }
      : credibilityScore >= 60
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                {
                  label: "Tests Passed",
                  value: approvedAttempts.filter((a) => a.passed).length,
                  icon: "✅",
                  c: "bg-emerald-50 text-emerald-600",
                },
                {
                  label: "Materials Done",
                  value: `${doneMats}/${totalMats}`,
                  icon: "📖",
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p
                            className={`text-sm font-bold ${a.passed ? "text-emerald-600" : "text-red-500"}`}
                          >
                            {a.percentage}%
                          </p>
                          <p className="text-xs text-gray-400">
                  ...
                  {topicRows.length === 0 ? (
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-50 border border-emerald-200">
                      ...
                      <p className="text-sm text-emerald-800 font-medium">
                        No active learning gaps. Great work!
                      </p>
                    </div>
                  ) : (
                      ...
                      {gaps.filter((g) => g.accuracy < 60).length > 0 && (
                        <div className="mt-3 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
                          ...
                          <p className="text-xs text-amber-800">
                            You have{" "}
                            <strong>
                              {gaps.filter((g) => g.accuracy < 60).length}
                            </strong>{" "}
                            topic
                            {gaps.filter((g) => g.accuracy < 60).length !== 1
                              ? "s"
                              : ""}{" "}
                            below 60% accuracy.{" "}
                            <Link
